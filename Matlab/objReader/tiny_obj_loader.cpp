  ...
  const std::string &name)
{
	...
	std::vector<float> normals;
	std::vector<float> texcoords;
	//nave: added face vertex/normals/textures indices
	std::vector< std::vector<int> >   positions_ind;
	std::vector< std::vector<int> >   normals_ind;
	std::vector< std::vector<int> >   texcoords_ind;
	...
	// nave: save face vertex/normals/textures indices
	bool isTriangleMesh = true, isQuadMesh = true;
	for (size_t i = 0; i < faceGroup.size(); i++) {
		std::vector<int> v_ind;
		std::vector<int> vt_ind;
		std::vector<int> vn_ind;
		const std::vector<vertex_index>& face = faceGroup[i];
		if(face.size() != 3) {
		...
		for (size_t j = 0; j < face.size(); j++) {
			v_ind.push_back(face[j].v_idx + 1);
			vn_ind.push_back(face[j].vn_idx + 1);
			vt_ind.push_back(face[j].vt_idx + 1);
		}
		positions_ind.push_back(v_ind);
		if(normals.size() > 0) {
			normals_ind.push_back(vn_ind);
		}
		if(texcoords.size() > 0) {
			texcoords_ind.push_back(vt_ind);
		}
	}
	...
	//
	shape.name = name;
	// nave: changed this line to keep the same order of the vertices before triangulation
	//shape.mesh.positions.swap(positions);
	//shape.mesh.normals.swap(normals);
	//shape.mesh.texcoords.swap(texcoords);

	// vertices normals and textures before triangulation
	shape.mesh.positions = in_positions;
	shape.mesh.normals = in_normals;
	shape.mesh.texcoords = in_texcoords;

	// nave: add new indices
	shape.mesh.face_vertex_ind.swap(positions_ind);
	shape.mesh.face_normals_ind.swap(normals_ind);
	shape.mesh.face_texcoords_ind.swap(texcoords_ind);

	shape.mesh.indices.swap(indices);

	// nave: add if quad or triangle mesh
	shape.mesh.isTriangleMesh = isTriangleMesh;
	shape.mesh.isQuadMesh = isQuadMesh;
